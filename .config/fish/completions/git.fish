...
function __fish_git_custom_commands
    ...
    set -l path_dirs (string split : -- $PATH)
    for dir in $path_dirs
        for file in $dir/git-*
            if test -x "$file"
                set -l name (string replace -r '^.*/git-' '' -- "$file")
                switch $name
...
complete -f -c git -n '__fish_git_using_command commit && not string match -rq -- "^(--fixup\b|--squash\b|-C|-c)" (commandline -opc)[-1]' -a '(__fish_git_files modified deleted modified-staged-deleted untracked)'
...
complete -f -c git -n '__fish_git_using_command commit' -n 'string match -rq -- "^(--fixup\b|--squash\b|-C|-c|--reuse-message\b|--reedit-message\b)" (commandline -opc)[-1]' -ka '(__fish_git_recent_commits)'
...
complete -c git -n '__fish_git_using_command diff' -n 'not contains -- -- (commandline -opc)' -ka '(__fish_git_recent_commits --all)'
...
complete -F -c git -n '__fish_git_using_command reset' -n 'contains -- -- (commandline -opc)'
