...
### 🧩 Obsidianの設定/プラグインをdotfilesで管理する（WSL vs ネイティブ 比較）

> 上の MCP連携は「Claude がノートを読み書きする」話。こちらは「**Obsidian本体の設定・プラグインを dotfiles にまとめる**」話で別物。

**採用方式: シンボリックリンク方式**（Home Managerの `programs.obsidian` モジュールは「設定を全部Nix式に変換」してしまい、本リポジトリの *ネイティブ設定を保つ* 哲学（`docs/ARCHITECTURE.md`）に反するため不採用）。
`.obsidian/` をネイティブ形式のまま `.config/obsidian/` に置き、既存の `seedCopilotConfig`（`home.nix`）と同型の **activation script** で `<vault>/.obsidian` へ seed/link する。

#### 何をコミットし、何を除外するか

| コミット推奨（共有設定） | gitignore推奨（マシン固有・状態/秘密） |
|---|---|
| `community-plugins.json` `core-plugins.json` `appearance.json` `hotkeys.json` `app.json`、`snippets/` `themes/`、`plugins/<id>/{main.js,manifest.json,styles.css}` | `workspace.json` `workspace-mobile.json`、`cache/`、`plugins/*/data.json`（APIキー/絶対パスを含む場合） |

#### ⚠️ 環境差分の核心（Machine1 ネイティブArch vs Machine2 Win11+WSL2）

`llm/context/environment.md` の通り、**Machine2 では Obsidianアプリが Windows 側**にある。これが「dotfilesで完全にまとめる」のを難しくする:

- Obsidian は `\\wsl$\` ネットワークパスの vault を**正式サポートしない**。
- vault を Windows FS（WSLから `/mnt/c/...`）に置くと、WSL の home-manager が作る **Linuxシンボリックリンクを Windows版 Obsidian が追えない**。さらに `/mnt/c` は symlink・パーミッション・git性能に難あり。
- → **WSL側の home-manager から Windows vault の `.obsidian` を綺麗に管理するのは困難**。

#### 3つの方式の比較（WSL環境向け）

| 方式 | 仕組み | メリット | デメリット |
|---|---|---|---|
| **① WSL2内でObsidianを動かす（WSLg）** ⭐おすすめ | vaultをLinux FSに置き、Machine1と同一の activation script で管理 | dotfilesで**完全統一**（両機で1つの仕組み）。symlinkそのまま | WindowsネイティブObsidian体験を手放す。WSLgのGUI/日本語IME/フォント設定が必要 |
| **② Windows側Obsidian + vaultをWindows FS**（現状維持） | `.obsidian` はWindows側のgitで管理（dotfilesを`/mnt/c`にもclone、または Obsidian Git プラグイン） | WindowsネイティブObsidian体験を維持 | 2環境で管理経路が分裂し**完全な"まとめ"にならない**。WSLのhome-managerは関与せず symlink不可 |
| **③ Windows Obsidian + WSLにvault + 双方向同期** | [ObsidianWSLSync](https://github.com/quercyAP/ObsidianWSLSync) でWin↔WSLをリアルタイム同期 | 両方の良いとこ取り | 同期の複雑さ・コンフリクト・追加プラグイン依存 |

#### 両環境で共通化できる / 差分が出る要素

- ✅ **共通化OK**: `plugins/<id>/main.js` は JS なので**OS非依存** → ベンダリング（コミット）すれば両機で再現可。`community-plugins.json` / `appearance.json` / `hotkeys.json` / themes / snippets も共通。
- ⚠️ **差分注意**:
  - vaultパスがOS/ホストで異なる → activation script で `uname -r`（または `/proc/version`）に `microsoft` を含むか判定して分岐。
  - `plugins/*/data.json` はマシン固有の絶対パス/秘密を含み得る → gitignore か per-host 管理。
  - **改行コード**: Win/Linuxで共有するJSONの CRLF/LF 混在を防ぐため、リポジトリに `.gitattributes`（**現状なし**）を追加し `.obsidian/**` を `eol=lf` に正規化。

#### 推奨

- 「**dotfilesにまとめたい**」が主目的なら → **①（WSL2内でObsidianを動かす）** が唯一の"完全統一"解。
- WindowsネイティブObsidianを維持したいなら → **②** とし、「WSL側はvault設定をdotfilesで完全管理はできない」制約を許容する。
- 優先度: Med（プラグイン本体のベンダリングはHigh価値・OS非依存で確実）
...
**Obsidian設定管理 / WSL連携**
- [home-manager: programs/obsidian.nix](https://github.com/nix-community/home-manager/blob/master/modules/programs/obsidian.nix)
- [Support for vaults in Windows Subsystem for Linux (WSL) — Obsidian Forum](https://forum.obsidian.md/t/support-for-vaults-in-windows-subsystem-for-linux-wsl/8580)
- [quercyAP/ObsidianWSLSync](https://github.com/quercyAP/ObsidianWSLSync)
